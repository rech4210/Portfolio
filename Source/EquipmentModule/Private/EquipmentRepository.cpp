...
void UEquipmentRepository::Initialize(IDBProviderInfra* Infra) {
	if (!Infra) {
		UE_LOG(LogTemp, Error, TEXT("EquipmentRepo : DBProviderInfra is null during initialization!"));
	}
	EquipmentDBProvider = Infra->GetEquipDbProvider();
	if (!EquipmentDBProvider.IsValid()) {
		UE_LOG(LogTemp, Error, TEXT("EquipmentRepo : EquipDbProvider is not available!"));
	}
}
