// @Needmodifi
// 1) Windows ?Ä???òÌçº & Îß§ÌÅ¨Î°?Ï∂©Îèå Î∞©Ï?
/* 
 * Î¨∏Ï†ú ?êÏù∏:
 * - Unreal Engine?Ä ?ÑÏ≤òÎ¶??®Í≥Ñ?êÏÑú 'check(...)' Îß§ÌÅ¨Î°úÎ? ?ïÏùò?òÍ≥† ?¨Ïö©??
 * - MySQL Connector/C++???ÑÏó≠ ?®Ïàò 'check(const std::string&)' ?±ÏùÑ ?ïÏùò??
 * - ???ïÏùòÍ∞Ä ?¥Î¶Ñ??Í≥µÏú†?òÎ©∞, include ?úÏÑú???∞Îùº ?úÎ°ú ?§Î•∏ Î∞©Ïãù?ºÎ°ú ?¥ÏÑù??
 *
 * include ?úÏÑú???∞Î•∏ Ï∞®Ïù¥:
 * 1) UE ?§Îçî Î®ºÏ? ?¨Ìï® ??Îß§ÌÅ¨Î°??ïÏùò ??#undef check ??MySQL ?§Îçî ?¨Ìï®
 *    - UE Îß§ÌÅ¨Î°úÍ? ?¥Î? ?¨Ïö©???¥ÌõÑ?¥Î?Î°?MySQL ?®Ïàò ?ïÏùòÎß??†Íπê Í∞Ä?§Ï†∏ Ï∂©Îèå ?ÜÏùå.
 * 2) MySQL ?§Îçî Î®ºÏ? ?¨Ìï® ???ÑÏó≠ ?®Ïàò ?ïÏùò ??#undef check ??UE ?§Îçî ?¨Ìï®
 *    - UE Îß§ÌÅ¨Î°úÍ? ?ÑÏßÅ ?ïÏùò?òÏ? ?äÏ? ?ÅÌÉú?¥Î?Î°??ÑÏó≠ ?®ÏàòÍ∞Ä ?®ÏïÑ UE ÏΩîÎìú??check ?∏Ï∂úÍ≥?Ï∂©Îèå.
 *
 * ?¥Í≤∞Ï±?
 * - MySQL ?§Îçî??Îß§ÌÅ¨Î°úÍ? ?úÍ±∞???ÅÌÉú?êÏÑúÎß??¨Ìï®?òÍ≥†,
 * - Unreal ?§Îçî?§Ï? MySQL ?§Îçî ?¥ÌõÑ 'CoreMinimal.h' ?±Ïóê??Îß§ÌÅ¨Î°úÍ? Î≥µÏõê???§Ïóê ?¨Ìï®?¥Ïïº ??
 */
#include "DatabaseManager.h"

// 3) ?∏Î¶¨??ÏΩîÏñ¥ ?§ÎçîÎ°?Îß§ÌÅ¨Î°?Î≥µÏõê
...
// ?¶PIMPL Íµ¨ÌòÑÏ≤??ïÏùò??
// C4150: Î∂àÏôÑ?ÑÌïú ?ïÏãù 'FDatabaseManagerImpl'???Ä???¨Ïù∏?∞Î? ??†ú?àÏäµ?àÎã§.
// -> PIMPL Íµ¨Ï°∞ ?¨Ïö©??unique_ptr?êÏÑú ?åÎ©∏??deleteÎ•??òÎäî?? Ïª¥Ìåå???úÏ†ê??FDatabaseManagerImpl???ïÏùòÍ∞Ä Î∂àÏôÑ?ÑÌïòÍ∏∞Ïóê ?¥Îãπ ?êÎü¨Í∞Ä Ï∂úÎ†•??
// ?¥Í≤∞ : FDatabaseManagerImpl Íµ¨Ï°∞Ï≤¥Î? ?ÑÏ†Ñ?òÍ≤å ?ïÏùò?òÍ±∞?? UniquePtrÎ•??¨Ïö©?òÏ? ?äÍ≥† raw ?¨Ïù∏?∞Î? ?¨Ïö©?òÏó¨ ?åÎ©∏?êÎ? ÏßÅÏ†ë Íµ¨ÌòÑ?©Îãà??
...
void UDatabaseManager::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	// [?îÎ≤ÑÍ∑?Î°úÍ∑∏ Ï∂îÍ?] ?¥Îñ§ ?§Ï†ï?ºÎ°ú ?ëÏÜç???úÎèÑ?òÎäîÏßÄ ?ïÏù∏?©Îãà??
	...
	// Í≤ΩÍ≥†: Î≥¥Ïïà???ÑÌï¥ ÎπÑÎ?Î≤àÌò∏???àÎ? Î°úÍ∑∏Î°?Ï∂úÎ†•?òÏ? ÎßàÏÑ∏??
	...
	try
	{
		...
		{
			//connectionPool??Ï¥àÍ∏∞?îÍ? ?¥Î£®?¥Ï°å?îÍ??
...
UE::Tasks::TTask<bool> UDatabaseManager::WithTransaction(F&& Function, const TCHAR* TaskLabel)
{
	return UE::Tasks::Launch(TaskLabel, [this, Function = Forward<F>(Function)]() mutable -> bool
	{
		...
		try
		{
			...
			//TryÍ∞Ä ?±Í≥µ?òÎ©¥ TransactionGuard??Ï¢ÖÎ£å?êÍ? ?∏Ï∂ú?òÏ? ?äÎäîÍ±¥Í??
		}
		catch (const sql::SQLException& e)
		{
...
int32 UPlayerIdHelper::ConvertUserIdToPlayerId(const FString& UserId)
{
	...
	{
		// ?´ÏûêÎß??àÎäî Í≤ΩÏö∞ ÏßÅÏ†ë Î≥Ä???úÎèÑ
		if (UserId.IsNumeric())
		{
...
bool UPlayerIdHelper::IsValidUserId(const FString& UserId)
{
	...
	// VARCHAR(255) ?úÌïú Í≤Ä??
	if (UserId.Len() > 255)
	{
		return false;
	}
	
	// Í∏∞Î≥∏?ÅÏù∏ Î¨∏Ïûê???†Ìö®??Í≤Ä??(Í≥µÎ∞±, ?πÏàòÎ¨∏Ïûê ??
...
{
	return SerializeInventoryItemData(SkillProperties); // Í∞ôÏ? Íµ¨Ï°∞ ?¨ÏÇ¨??
}

TMap<FString, FString> UDatabaseJsonHelper::DeserializeSkillData(const FString& JsonData)
{
	return DeserializeInventoryItemData(JsonData); // Í∞ôÏ? Íµ¨Ï°∞ ?¨ÏÇ¨??
}
...
UE::Tasks::TTask<TArray<FSkillSlotDatabaseDTO>> UDatabaseManager::LoadUserSkillSlots(const FString& UserId, const FString& SlotKey)
{
	return UE::Tasks::Launch(UE_SOURCE_LOCATION, [this, UserId, SlotKey]() -> TArray<FSkillSlotDatabaseDTO>
	{
		...
		try
		{
			...
			// SQL Query: user_skill_slots?Ä user_skillsÎ•?Ï°∞Ïù∏?òÏó¨ ?∞Ïù¥??Î°úÎìú
			FString Query;
			if (SlotKey.IsEmpty())
			{
			...
			Stmt->setString(1, TCHAR_TO_UTF8(*UserId)); // UserIdÎ•?FString?ºÎ°ú Ï≤òÎ¶¨
			if (!SlotKey.IsEmpty())
			{
			...
			while (Result->next())
			{
				...
				SlotDTO.UserId = UTF8_TO_TCHAR(Result->getString("user_id").c_str()); // FString?ºÎ°ú Ï≤òÎ¶¨
...
UE::Tasks::TTask<bool> UDatabaseManager::SaveUserSkillSlots(const TArray<FSkillSlotDatabaseDTO>& SkillSlotDTOs)
{
	return UE::Tasks::Launch(UE_SOURCE_LOCATION, [this, SkillSlotDTOs]() -> bool
	{
		...
		try
		{
			...
			// UPSERT ÏøºÎ¶¨ (INSERT ... ON DUPLICATE KEY UPDATE)
			FString Query = TEXT(
	...
	const FDateTime& EndDate)
{
	return UE::Tasks::Launch(UE_SOURCE_LOCATION, [this, UserId, SkillId, StartDate, EndDate]() -> TMap<int32, int32>
	{
		...
		try
		{
			...
			// ?µÍ≥Ñ ÏøºÎ¶¨ (Í∞ÑÎã®??Î≤ÑÏ†Ñ - ?§Ï†úÎ°úÎäî Î≥ÑÎèÑ??usage_logs ?åÏù¥Î∏îÏù¥ ?ÑÏöî?????àÏùå)
			FString Query = TEXT(
