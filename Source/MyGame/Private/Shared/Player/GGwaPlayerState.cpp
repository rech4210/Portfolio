...
AGGwaPlayerState::AGGwaPlayerState() {
	/*?§Ïßà?ÅÏù∏ ?åÎ†à?¥Ïñ¥Í∞Ä ?¨Ïö©?òÍ≤å ??Ïª¥Ìè¨?åÌä∏ Í≤åÏûÑ Î°úÏßÅ Ï≤¥ÌÅ¨?Ä Î©îÎ™®Î¶?Î≥ÄÍ≤ΩÏùÑ ?¥Îãπ*/
	...
	/*Í∑????ÅÏÜç?? DB Ï∂îÏÉÅ Í≥ÑÏ∏µ?Ä SubSystem - Repository (var DBmanager) Î°?Ï≤òÎ¶¨??Í≤?*/
}
...
void AGGwaPlayerState::BroadcastAttributeChange(const FGameplayAttribute& Attribute, float NewValue) const{
	...
	// value ?êÏ†ï??helperÎ°?
	// FObservedAttributeHelper<> Player??status enum ?ÑÏöî
	if (HasAuthority()) {
...
//server? client? Deprecated -> tag Í∞úÎÖê?ºÎ°ú ASC???ÑÏûÑ?òÏó¨ ?ÅÌÉú Ï≤òÎ¶¨Ï§?
